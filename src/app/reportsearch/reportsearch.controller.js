...
module.controller("ReportSearchController", ['$scope', '$state', '$rootScope', '$timeout', '$mdpDatePicker', '$mdpTimePicker', 'Alert', 'ReportSearchService', 'constants',
    function ($scope, $state, $rootScope, $timeout, $mdpDatePicker, $mdpTimePicker, $alert, reportSearchService, $const) {
        ...
        $scope.formSearch = {
            ...
            carriageNum: 'all',
            //行车报表查看电机时，应把首尾无电机的车厢屏蔽掉，不让用户选择
            ...
            switch ($scope.formSearch.selectType) {
                case 1:
                    ...
                    var a = $('<a/>', {
                        ...
                        download: $scope.formSearch.trainId + '_振动报表.csv'
                    }).appendTo('body');
                    a[0].click();
                    a.remove();
                    break;
                case 2:
                    ...
                    var a = $('<a/>', {
                        style: 'display:none',
                        href: 'data:application/octet-stream;base64,' + btoa(unescape(encodeURIComponent(csvString))),
                        download: $scope.formSearch.trainId + '_轴温报表.csv'
                    }).appendTo('body');
                    a[0].click();
                    a.remove();
                    break;
                case 3:
                    ...
                    var a = $('<a/>', {
                        style: 'display:none',
                        href: 'data:application/octet-stream;base64,' + btoa(unescape(encodeURIComponent(csvString))),
                        download: $scope.formSearch.trainId + '_电机报表.csv'
                    }).appendTo('body');
                    a[0].click();
                    a.remove();
                    break;
